...
dt_train_block <- read_rds("cache/02_dt_train_block.rds") %>%
  select(label:rad_an) %>%
  mutate(across(-label, my_scale)) %>%
  mutate(across(label, ~ if_else(.x == -1, 0, .x)))

dt_val_block <- read_rds("cache/02_dt_val_block.rds") %>%
  select(label:rad_an) %>%
  mutate(across(-label, my_scale)) %>%
  mutate(across(label, ~ if_else(.x == -1, 0, .x)))

dt_train_kmeans <- read_rds("cache/02_dt_train_kmeans.rds") %>%
  select(label:rad_an) %>%
  mutate(across(-label, my_scale)) %>%
  mutate(across(label, ~ if_else(.x == -1, 0, .x)))

dt_val_kmeans <- read_rds("cache/02_dt_val_kmeans.rds") %>%
  select(label:rad_an) %>%
  mutate(across(-label, my_scale)) %>%
  mutate(across(label, ~ if_else(.x == -1, 0, .x)))
